    ...
    await messages.forEach(msg => {
        ctx.reply(
            ...
            Markup.inlineKeyboard([
                Markup.button.webApp('Open chat', `https://71b7-35-205-71-138.ngrok-free.app/chat?profile_id=${msg.profileId}&name=${msg.name}`)
            ])
        );
    });
});
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', async (message) => {
        ...
        if (data.userName && data.profileId) {
            ws.chat = new Chat(token, data.profileId);
