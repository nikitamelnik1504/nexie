...
export default class Account {
  constructor(token, profileId, login = null, password = null) {
  ...
  async fancentroAuth() {
    try {
      ...
      await this.page.evaluate(() => {
        document.querySelectorAll('button span[data-i18context="snapcentro_authorize_login"]')[1].parentElement.click()
      })
    ...
    try {
      ...
      try {
        ...
        if (await this.page.$('.avatar-container.pointer')) {
          ...
          const modelName = await this.page.evaluate(() => {
            return document.querySelector('.display-name').innerText
          });
          await this.page?.close();
          return {success: true, name: modelName};
        }
  ...
  async getAuthorizedAccountTon() {
    try {
      ...
      try {
        ...
        const modelName = await this.page.evaluate(() => {
          return document.querySelector('.Other__profile__name').textContent
        })
        await this.page?.close();
        return {success: true, name: modelName};
      } catch {
