...
class StartScene extends TelegramBotSceneBase {
  ...
  static async enterCommand(service, context) {
    const user = await (await service.getStorage()).getUser(context.from.username);

    if (user.role === 'admin') {
      return context.reply('Welcome!', Markup.keyboard(['Settings']).resize().oneTime());
    } else if (user.role === 'default') {
      return context.reply('Welcome!', Markup.keyboard([Markup.button.webApp('Chats', 'https://google.com'), 'Accounts']).resize().oneTime());
    }
  }
