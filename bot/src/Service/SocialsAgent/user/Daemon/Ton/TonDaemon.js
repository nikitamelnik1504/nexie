...
/*    if (instance.watchers.account.getAccount().username !== username) {
    ...
    this.eventEmitter = new class extends EventEmitter {
      ...
      sendMessage(dialogId, message, attachments = [], charge = {}, _bag = {}) {
        instance.watchers.messages.watcher.sendMessage(dialogId, message, attachments, charge, _bag);
      }
    }();
    ...
    this.watchers.messages.watcher.on("messagesList", (data) => {
      ...
      if (this.watchers.messages.store.requestMessages[data.memberId].queue !== 0) {
        ...
        this.eventEmitter.emit('messagesList', { memberId: data.memberId, data: this.watchers.messages.store.requestMessages[data.memberId].cache });
    ...
    this.watchers.messages.watcher.on("messageSent", (data, _bag) => {
      this.eventEmitter.emit('messageSent', data, _bag);
    });
    ...
    this.watchers.media.watcher.on("albumMediasList", (data) => {
      this.eventEmitter.emit('albumMediasList', { albumId: data.albumId, data: data.list });
    });
