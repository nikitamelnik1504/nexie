...
class MessagesCollection extends MessagesCollectionBase {
  ...
  static create(_messenger, _dialog, clientBrowserDaemonEventEmitter) {
    ...
    instance.clientBrowserDaemonEventEmitter.on('messagesList:' + instance._dialog.remote.id, (data) => {
  ...
  list(offset = 0, limit = 30) {
    ...
    if (itemsToReturn.length < limit) {
      ...
      this.clientBrowserDaemonEventEmitter.requestMessages(this._dialog.remote.id, remainingMessages);
    }
