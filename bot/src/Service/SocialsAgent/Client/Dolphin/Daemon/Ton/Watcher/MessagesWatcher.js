...
class MessagesWatcher extends EventEmitter {
  ...
  async requestMessages(memberId, startFrom) {
    return this.page.evaluate(async (accessToken, memberId, startFrom) => {
      const requestBody = {
        startFrom,
      };

      fetch('https://api.ton.place/im/' + memberId + '/history', {
        method: 'POST',
        headers: {
    ...
    }, this.account.token, memberId, startFrom);
  }

  async sendMessage(memberId, message, _bag) {
    return this.page.evaluate(async (accessToken, memberId, message, _bag) => {
      const requestBody = {
        attachments: [],
        randomId: -1,
        text: message
      ...
      fetch('https://api.ton.place/im/' + memberId + '/send', {
        method: 'POST',
        headers: {
          'Authorization': accessToken,
          'Content-Type': 'application/json',
    ...
    }, this.account.token, memberId, message, _bag);
  }
