class AccountWatcher {
  ...
  static async init(page) {
    ...
    instance.page.on('request', async (request) => {
      if (request.url() === 'https://api.ton.place/main/init' && request.headers().authorization) {
    ...
    instance.page.on('response', async (response) => {
      if (response.url() === 'https://api.ton.place/main/init' && response.headers()['content-type'] === 'text/plain; charset=utf-8') {
