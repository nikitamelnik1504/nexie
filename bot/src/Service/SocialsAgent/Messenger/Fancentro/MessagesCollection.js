...
class MessagesCollection extends MessagesCollectionBase {
  ...
  static create(_messenger, _dialog, browserDaemonEventEmitter) {
    ...
    instance.browserDaemonEventEmitter.on('messagesList:' + instance._dialog.remote.id, (data) => {
    ...
    instance.browserDaemonEventEmitter.on('messageNew:' + instance._dialog.remote.id, (data) => {
  ...
  list(offset = 0, limit = 30) {
    ...
    if (itemsToReturn.length < limit) {
      ...
      this.browserDaemonEventEmitter.requestMessages(this._dialog.remote.id, remainingMessages);
    }
